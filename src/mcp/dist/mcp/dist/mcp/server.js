...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (uri === 'pr-summaries://pending') {
        ...
        if (error)
            throw new Error(`Failed to fetch pending PRs: ${error.message}`);
        return {
            contents: [
                {
                    uri,
    ...
    if (uri === 'pr-summaries://approved') {
        ...
        if (error)
            throw new Error(`Failed to fetch approved PRs: ${error.message}`);
        return {
            contents: [
                {
                    uri,
    ...
    if (uri === 'linear-tickets://pending') {
        const { data, error } = await supabaseAdmin
            ...
            .order('created_at', { ascending: false });
        if (error)
            throw new Error(`Failed to fetch pending tickets: ${error.message}`);
        return {
            contents: [
                {
                    uri,
    ...
    if (uri === 'linear-tickets://approved') {
        const { data, error } = await supabaseAdmin
            ...
            .order('approved_at', { ascending: false });
        if (error)
            throw new Error(`Failed to fetch approved tickets: ${error.message}`);
        return {
            contents: [
                {
                    uri,
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === 'approve_summary') {
        const { id, type, edited_summary, approved_by = 'MCP User' } = args;
    ...
    if (name === 'reject_summary') {
        const { id, type, approved_by = 'MCP User' } = args;
        const table = type === 'pr' ? 'pr_summaries' : 'linear_tickets';
        const { data, error } = await supabaseAdmin
            .from(table)
            .update({
    ...
    if (name === 'search_summaries') {
        const { query, type = 'all', status = 'all' } = args;
        let results = [];
        // Search PR summaries
        if (type === 'all' || type === 'pr') {
